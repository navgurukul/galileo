...
class ReportController {
    ...
    //
    // }
    getStudentReport(request, h) {
        return new Promise((resolve, reject) => {
            let submissionsObj = {};
            let submissionsList = [];
            __1.default("submissions")
                .select("submissions.id", "submissions.exercise_id", "submissions.user_id", "submissions.submitted_at", "submissions.submitter_notes", "submissions.files", "submissions.state", "submissions.completed", "submissions.completed_at", "exercises.name")
                .innerJoin("exercises", "submissions.exercise_id", "exercises.id")
                .where({
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    */
                    ...
                    let curSubState = subStateOrder.indexOf(submission.state);
                    let storedSubmission = submissionsObj[submission.exercise_id] || {};
                    // No submission is stored in the object
                    if (Object.keys(storedSubmission).length === 0) {
                        submissionsObj[submission.exercise_id] = submission;
                        submissionsObj[submission.exercise_id]["attempts"] = 1;
                    }
                    // If the submission is stored
                    else {
                        let storedSubState = subStateOrder.indexOf(storedSubmission.state);
                        let attempts = submissionsObj[submission.exercise_id]["attempts"] + 1;
                        // Replace the stored submission with the current submission if
                        // the stored one is of a lesser level
                        if (storedSubState < curSubState) {
                            submissionsObj[submission.exercise_id] = submission;
                        }
                        submissionsObj[submission.exercise_id]["attempts"] = attempts;
                    }
                }
    ...
    getMenteesCoursesReport(request, h) {
        //request.user_id=175;
        return new Promise((resolve, reject) => {
            request.user_id = 1;
            let mentees = [], menteesCoursesReport = [];
            __1.default("user_roles")
                ...
                const totalExercisesQ = "COUNT(CASE WHEN exercises.submission_type " +
                ...
                if (response.isFacilitator === true) {
                    ...
                    menteesReportQ = __1.default("course_enrolments")
                        .select(
                    // course details in which the mentee has enrolled
                    "courses.name as courseName", "courses.id as course_id", "course_enrolments.course_status as menteeCourseStatus", __1.default.raw(totalExercisesQ), __1.default.raw(completedSubmissionsQ), 
                    // mentees details
                    "users.name as menteeName", "users.email as menteeEmail")
                        .innerJoin("courses", "courses.id", "course_enrolments.course_id")
                        .innerJoin("users", "course_enrolments.student_id", "users.id")
                        .innerJoin("user_roles", "user_roles.user_id", "users.id")
                        .innerJoin("exercises", "course_enrolments.course_id", "exercises.course_id")
                        .leftJoin("submissions", function () {
                        this.on("submissions.user_id", "=", "users.id")
                            .andOn("submissions.exercise_id", "=", "exercises.id")
                    ...
                    "users.name as menteeName", "users.email as menteeEmail")
                        .innerJoin("courses", "courses.id", "course_enrolments.course_id")
                        .innerJoin("mentors", "course_enrolments.student_id", "mentors.mentee")
                        .innerJoin("users", "users.id", "mentors.mentee")
                        .innerJoin("exercises", "course_enrolments.course_id", "exercises.course_id")
                        .leftJoin("submissions", function () {
                        this.on("submissions.user_id", "=", "mentors.mentee")
                            .andOn("submissions.exercise_id", "=", "exercises.id")
                            .andOn("submissions.completed", "=", 1);
                    })
                        .where({
                ...
                for (let i = 0; i < menteesCoursesReport.length; i++) {
                    const _a = menteesCoursesReport[i], { courseName, course_id, totalExercises } = _a, userDetails = __rest(_a, ["courseName", "course_id", "totalExercises"]);
                ...
                for (let courseName of Object.keys(courses)) {
                    let courseReport = Object.assign({ courseName }, courses[courseName]);
    ...
    getMenteesExercisesReport(request, h) {
        console.log('--------------getMenteesExercisesReport----------------');
        //request.user_id = 175;
        return new Promise((resolve, reject) => {
            let mentees = [], menteeSubmissions = [], exercises = {};
            __1.default("user_roles")
                .select("*")
                .where({
                "user_roles.user_id": request.user_id,
                ...
                return __1.default("courses")
                    .select("courses.id as course_id", "courses.name as courseName ", "courses.type as courseType", "courses.logo as courseLogo", "courses.short_description as courseShortDescription")
                    ...
                    if (rows.length < 1) {
                        ...
                        return Promise.resolve(Object.assign(Object.assign({}, response), { courseData: rows[0] }));
                    }
                });
            })
                .then(response => {
                ...
                exerciseQ = __1.default("exercises")
                    .select("exercises.id as exercise_id", "exercises.slug as exerciseSlug", "exercises.content as exerciseContent", "exercises.sequence_num as exerciseSequenceNum", "exercises.name as exerciseName", "exercises.submission_type as exerciseSubmissionType", "exercises.github_link as exerciseGithubLink")
                    ...
                    for (let i = 0; i < rows.length; i++) {
                        ...
                        exercises[exercise.exercise_id]["submissions"] = [];
                    }
                });
                ...
                if (response.isFacilitator === true) {
                    ...
                    submissionQ = __1.default("submissions")
                        .select("submissions.id as submissionId", "submissions.state as submissionState", "submissions.submitter_notes as submitter_notes", "submissions.completed as submissionCompleted", "submissions.exercise_id as exercise_id", "users.id as menteeId", "users.name as menteeName", "users.email as menteeEmail")
                        .innerJoin("exercises", "exercises.id", "submissions.exercise_id")
                        .innerJoin("users", "users.id", "submissions.user_id")
                        .innerJoin("user_roles", "user_roles.user_id", "users.id")
                        .where(Object.assign(Object.assign({}, whereClause), { "exercises.course_id": response.courseData.course_id }));
                    console.log();
                }
                else {
                    submissionQ = __1.default("submissions")
                        .select("submissions.id as submissionId", "submissions.submitter_notes as submitter_notes", "submissions.state as submissionState", "submissions.completed as submissionCompleted", "submissions.exercise_id as exercise_id", "users.id as menteeId", "users.name as menteeName", "users.email as menteeEmail")
                        .innerJoin("exercises", "exercises.id", "submissions.exercise_id")
                        .innerJoin("mentors", "mentors.mentee", "submissions.user_id")
                        .innerJoin("users", "users.id", "mentors.mentee")
                        .where({
                        "exercises.course_id": response.courseData.course_id,
                        "mentors.mentor": request.user_id
                ...
                submissionQ.then(rows => {
                    ...
                    for (let i = 0; i < rows.length; i++) {
                        let _a = rows[i], { exercise_id } = _a, submission = __rest(_a, ["exercise_id"]);
                        exercises[exercise_id]["submissions"].push(submission);
                    }
                    ...
                    // 
                    resolve(Object.assign(Object.assign({}, response.courseData), { menteesExercisesReport: menteeSubmissions, mentees: mentees }));
                });
            });
        });
    }
     ...
     * @param h
     */
    numberOfAssignmentSubmitted(request, h) {
        return new Promise((resolve, reject) => {
            let numberOfPendingRequests, requestTodays, requestYesterday, requestLastWeek, requestLastMonth;
            let totalRecord = reportHelper_1.getforIndivisualTimePeriod(request.query.centerId, null).then((results) => {
                numberOfPendingRequests = results;
                return Promise.resolve(numberOfPendingRequests);
            });
            let todaysRecord = reportHelper_1.getforIndivisualTimePeriod(request.query.centerId, 'today').then((results) => {
                requestTodays = results;
                return Promise.resolve(requestTodays);
            });
            let yesterdayRecord = reportHelper_1.getforIndivisualTimePeriod(request.query.centerId, 'yesterday').then((results) => {
                requestYesterday = results;
                return Promise.resolve(requestYesterday);
            });
            let lastWeekRecord = reportHelper_1.getforIndivisualTimePeriod(request.query.centerId, 'lastWeek').then((results) => {
                requestLastWeek = results;
                return Promise.resolve(requestLastWeek);
            });
            let lastMonthRecord = reportHelper_1.getforIndivisualTimePeriod(request.query.centerId, 'lastMonth').then((results) => {
     ...
     * @param h
     */
    numberOfAssignmentSubmittedPerUser(request, h) {
        return new Promise((resolve, reject) => {
            reportHelper_1.getNumberOfAssignmentSubmittedPerUser(request.query.centerId).then(rows => {
                // check if he is a facilitator?
                if (rows.length < 1) {
                    resolve([{
                            name: "No Students",
     ...
     * @param h
     */
    sendSubmissionReport(request, h) {
        return new Promise((resolve, reject) => {
            //  
            let userWiseDharmshalaCount, userWiseBangaloreCount, userWiseAllCount, totalDharmshalaCount, totalBangaloreCount, totalAllCount;
            ...
            Promise.all([
                ...
                let scheduleConf = configurations_1.getScheduleConfigs();
                ...
                let response = sendEmail_1.sendSubmissionReport(student, result);
                response.then(res => {
                    if (res == 'sent') {
