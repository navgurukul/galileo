...
const course_schemas_1 = require("./course-schemas");
...
function default_1(server, serverConfigs, database) {
    ...
    server.route({
        ...
        config: {
            ...
            response: {
                schema: Joi.object({
                    "availableCourses": Joi.array().items(course_schemas_1.courseSchema),
                    "enrolledCourses": Joi.array().items(course_schemas_1.enrolledCourseSchema),
                    "completedCourses": Joi.array().items(course_schemas_1.completedCoursesSchema),
                })
            },
            auth: {
                strategy: 'jwt',
    ...
    server.route({
        ...
        config: {
            ...
            response: {
                schema: Joi.object({
                    "data": Joi.array().items(course_schemas_1.topicSchema)
                })
            },
            tags: ['api'],
    ...
    server.route({
        ...
        config: {
            ...
            response: {
                schema: Joi.object({
                    "data": Joi.array().items(course_schemas_1.exerciseSchema)
                })
            },
            auth: {
                strategy: 'jwt',
    ...
    server.route({
        ...
        config: {
            ...
            response: {
                schema: Joi.object({
                    "data": Joi.array().items(user_schemas_1.userRoleSchema)
                })
            },
            auth: 'jwt',
            tags: ['api'],
