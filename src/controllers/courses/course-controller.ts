        ...
        return new Promise((resolve, reject) => {
            database("user_roles")
                ...
                .then(rows => {
                    ...
                    const isAdmin =
                        rows.length > 0 && access.isAdmin === true
                            ? true
                            : false;
                    const isFacilitator =
                        rows.length > 0 && access.isFacilitator === true
                            ? true
                            : false;
                    const isTnp =
                        rows.length > 0 && access.isTnp === true ? true : false;
                    const userRole =
                        rows.length > 0 && access.roles !== undefined
                            ? access.roles
                            : false;
