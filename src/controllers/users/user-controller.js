...
class UserController {
    ...
    constructor(configs, database) {
        ...
        this.userModel = new user_model_1.UserModel(this.configs);
    ...
    loginUser(request, h) {
        ...
        return new Promise((resolve, reject) => {
            client.verifyIdToken(request.payload.idToken, this.configs.googleAuth.clientId, (error, login) => {
                ...
                let isAdmin = false, isAlumni = false;
                let userObj = {
                ...
                this.userModel.upsert(userObj, { 'email': userObj['email'] }, true)
                    .then((user) => {
                    return __1.default('user_roles').select('*')
                    ...
                    if (shouldCreateRole === true) {
                        ...
                        ;
                        return __1.default('user_roles').insert(userRoles)
                            .then(() => {
                            return Promise.resolve(Object.assign(Object.assign({}, user), { isAdmin,
                                isFacilitator,
                                isAlumni }));
                        });
                    }
                        ...
                        let shouldCreateFacilitatorRole = __1.default('user_roles').select('*')
                            .where({
                            ...
                            if (rows.length < 1 && isFacilitator) {
                                ...
                                return __1.default('user_roles').where({
                                    'user_roles.roles': 'facilitator',
                                    'user_roles.user_id': user.id,
                                    'user_roles.center': 'all'
                                })
                        ...
                        return shouldCreateFacilitatorRole
                            ...
                            return Promise.resolve(Object.assign(Object.assign({}, user), { isFacilitator,
                                isAdmin,
                                isAlumni }));
                        });
                    }
    ...
    updateUserInfo(request, h) {
        ...
        return new Promise((resolve, reject) => {
            ...
            var imagepath = "img/avatar/avatar_" + request.user_id + "." + extension;
                ...
                fs.readFile(imagepath, function (err, data) {
                    ...
                    s3.upload(params, function (err, data) {
                        if (err) {
                            ...
                            //
                            that.userModel
                                .upsert(userDeatils, { id: request.params.user_id }, true)
                                .then(user => {
                                resolve({ user: user });
                            });
                        }
