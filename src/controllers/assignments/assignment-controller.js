...
class AssignmentController {
    ...
    postExerciseSubmission(request, h) {
            ...
            __1.default("course_enrolments")
                ...
                if (response.canSubmit === true) {
                    ...
                    __1.default("exercises")
                        ...
                        if (exercise.review_type === "manual") {
                        ...
                        else if (exercise.review_type === "automatic") {
                            /** Not in work */
                            return Promise.resolve({
                                exercise_id: request.params.exerciseId,
                                user_id: request.user_id,
                            ...
                            if (exercise.review_type === "peer") {
                                reviewerIdQuery = __1.default("submissions")
                                    .select("submissions.user_id as reviewerID")
                                    .innerJoin("course_enrolments", "submissions.user_id", "course_enrolments.student_id")
                                    .where({
                            ...
                            return reviewerIdQuery
                                ...
                                return Promise.resolve({
                                    ...
                                    state: "pending",
                                    completed: 0,
                                    peer_reviewer_id: response.reviewerId
                                });
                            });
                        }
                    })
                        ...
                        if (count >= 1) {
                            ...
                            submissionInsertQuery = __1.default("submissions")
                                ...
                                .then(row => {
                                return Promise.resolve({
                                    reviewerId: queryData.peer_reviewer_id,
                        ...
                        submissionInsertQuery
                             ...
                             */
                            let studentQuery = __1.default("users")
                            ...
                            let reviewerQuery = __1.default("users")
                                .select("users.email", "users.name")
                                .where({
                            ...
                            return Promise.all([
                                ...
                                "submissions.id as submissionId", "exercises.name as exerciseName", "exercises.slug as exerciseSlug", "submissions.state", "submissions.completed")
                                    .innerJoin("exercises", "exercises.id", "submissions.exercise_id")
                                    .where({
                                    "submissions.user_id": request.user_id,
                                    exercise_id: request.params
                                        .exerciseId
                                });
                            });
                        })
    ...
    uploadExerciseAssignment(request, h) {
    ...
    getExerciseSubmissions(request, h) {
        return new Promise((resolve, reject) => {
            // query to find all the submission of particular exercise_id
            let submissionQuery = __1.default("submissions")
                .select(
            // Submissions table fields
            "submissions.id", "submissions.exercise_id", "submissions.submitted_at", "submissions.submitter_notes", "submissions.files", "submissions.notes_reviewer", "submissions.state", "submissions.completed", "submissions.completed_at", 
            // Reviewer details
            ...
            // Submitter Details
            "users.name as submitterName", "users.id as submitterId", "users.profile_picture as submitterProfilePicture"
    ...
    getExerciseSubmissionById(request, h) {
        return new Promise((resolve, reject) => {
            // for searching single assignment submission by submissionId
            __1.default("submissions")
                .select("submissions.id", "submissions.exercise_id", "submissions.user_id", "submissions.submitted_at", "submissions.submitter_notes", "submissions.files", "submissions.notes_reviewer", "submissions.state", "submissions.completed", "submissions.completed_at", "submissions.submitted_at", "users.name as reviewerName", "users.id as reviewerId", "users.profile_picture as reviewerProfilePicture")
    ...
    getPeerReviewRequests(request, h) {
        return new Promise((resolve, reject) => {
            ...
            __1.default("users")
                ...
                __1.default("submissions")
                    .select(
                // Submissions table
                "submissions.id", "submissions.exercise_id", "submissions.submitted_at", "submissions.submitter_notes", "submissions.files", "submissions.notes_reviewer", "submissions.state", "submissions.completed", "submissions.completed_at", "submissions.submitted_at", 
                // Exercises Table
                "exercises.id as exercise_id", "exercises.parent_exercise_id as parent_exercise_id", "exercises.course_id", "exercises.name as exerciseName", "exercises.slug as exerciseSlug", "exercises.sequence_num as exerciseSequenceNum", "exercises.review_type", "exercises.content as exerciseContent", 
                // Users table
                "users.id as submitterId", "users.name as submitterName", "users.profile_picture as submitterProfilePicture"
                // 'users.facilitator as isSubmitterFacilitator'
                )
    ...
    editPeerReviewRequest(request, h) {
        //const usersId = 29;
        //request.user_id = 29;
        ...
        return new Promise((resolve, reject) => {
            ...
            __1.default("submissions")
                .select("submissions.id", "submissions.user_id", "submissions.state", "submissions.peer_reviewer_id", "submissions.exercise_id", "submissions.Id", "mentors.mentor", "users.center")
                ...
                .then((submission) => {
                __1.default('exercises').select('course_id').where({ 'exercises.id': submission.exercise_id }).then((rows) => {
                    ...
                    __1.default("course_enrolments")
                        ...
                        if (response.isAlreadyEnrolled) {
                            __1.default("submissions")
                                .select("submissions.id", "submissions.user_id", "submissions.state", "submissions.peer_reviewer_id", "submissions.Id", "mentors.mentor", "users.center")
                                .leftJoin("mentors", "user_id", "mentee")
                                .innerJoin("users", "submissions.user_id", "users.id")
                                .where({
                                "submissions.id": request.params.submissionId
                            })
                                .then(rows => {
                                ...
                                return __1.default("user_roles")
                                    ...
                                    .whereIn("center", [submission.center, "all"] // or where the center is all
                                )
                                    .then(rows => {
                                    ...
                                    if (usersId ===
                                        ...
                                        if (request.payload.approved) {
                                            ...
                                            updateFields["mark_completed_by"] = usersId;
                                        }
                                        else {
                                ...
                                if (isAssigmentApproved) {
                                    //
                                    this.checkDependencyCourses(request.user_id).then(courses => {
                                        ...
                                        __1.default("submissions")
                                            ...
                                            this.checkDependencyCourses(request.user_id).then(courses => {
                                                ...
                                                this.checkIfDependencyCourseUnlocked(initialAvailableCourses, availableCoursesPostAssigmentApproval, request.user_id);
                                                /*  Finding the student and the reviewer details to send email
                                                 * to them about the assignment review.
                                                 */
                                                let student, reviewer;
                                                let studentQ = __1.default("users")
                                                    .select("*")
                                                    .where({
                                                    "users.id": submission.user_id
                                                })
                                                    .then(rows => {
                                                    student =
                                                        rows[0];
                                                ...
                                                let reviewerQ = __1.default("users")
                                                    .select("*")
                                                    .where({
                                                    "users.id": submission.user_id
                                                })
                                                ...
                                                return Promise.all([
                                                    ...
                                                    return __1.default("submissions")
                                                        .select("exercises.course_id", "exercises.slug", "exercises.name")
                                                        .innerJoin("exercises", "exercises.id", "submissions.exercise_id")
                                                        .where({
    ...
    checkDependencyCourses(user_id) {
        ...
        return new Promise((resolve, reject) => {
            __1.default("courses")
                ...
                this.on("courses.id", "=", "course_enrolments.course_id").andOn("course_enrolments.student_id", "=", user_id);
            }))
                .then(rows => {
                ...
                __1.default("exercises")
                    ...
                    __1.default("exercises")
                        ...
                        .andWhere("submissions.user_id", "=", user_id) //******replace 9 with request.user_id*****//
                    )
                        .groupBy("exercises.course_id")
                        .then(rows => {
                        ...
                        __1.default("course_relation")
                            ...
                            //
                            allAvailableCourses =
                                mergedResult["allAvailableCourses"];
                            exerciseCompeletedPerCourse =
                                mergedResult["exerciseCompeletedPerCourse"];
                            totalExercisesPerCourse =
                                mergedResult["totalExercisesPerCourse"];
                            courseReliesOn =
                                mergedResult["courseReliesOn"];
                            //
                            //
                            //
                            availableCourses = courseHelper_1.manipulateResultSet(totalExercisesPerCourse, exerciseCompeletedPerCourse, courseReliesOn, allAvailableCourses, courseConfig.courseCompleteionCriteria);
    ...
    checkIfDependencyCourseUnlocked(initialAvailableCourses, availableCoursesPostAssigmentApproval, user_id) {
    ...
    ProcessEmailNotification(unlockedCourses, user_id) {
        let student, courses;
        let cousresQ = __1.default("courses")
            .select("name")
