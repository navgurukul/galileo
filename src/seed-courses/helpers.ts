    ...
    for (var i = 0; i < globals.exercises.length; i++) {
        ...
        exPromises.push(_uploadContentImages(exercise, i).then((uploadedImages) => {
            if (uploadedImages.length) {
                ...
                for (let exContent in exercise['content']) {
                    content = exercise['content'][exContent];
                    globals.exercises[iIndex]['content'][exContent] = updateContentWithImageLinks(uploadedImages, content);
                }
            };
        }));
