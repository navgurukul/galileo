...
exports.uploadImagesAndUpdateContent = exports.getAllExercises = exports.parseNgMetaText = exports.getCurriculumExerciseFiles = exports.getSequenceNumbers = void 0;
...
exports.getSequenceNumbers = function (dir, callType) {
...
exports.getCurriculumExerciseFiles = function (dir, callType) {
...
exports.parseNgMetaText = function (text) {
...
let _getExerciseInfo = function (path, sequence_num, isSolutionFile) {
    ...
    if (tokens.length < 1) {
        utils_1.showErrorAndExit("No proper markdown content found in " + path);
    }
    ...
    if (tokens[0].type !== 'code' || tokens[0].lang !== 'ngMeta') {
        ...
        exInfo = exports.parseNgMetaText(tokens[0]['text']);
        try {
            ...
            utils_1.showErrorAndExit(`There is some error in ${path}`);
        }
        if (!exInfo['completionMethod']) {
            exInfo['completionMethod'] = 'peer';
        }
    }
    exInfo = Joi.attempt(exInfo, schema_1.exerciseInfoSchema);
...
exports.getAllExercises = function (exercises) {
    let exerciseInfos = [];
    for (let i = 0; i < exercises.length; i++) {
        ...
        if (exercises[i].childExercises.length > 0) {
            let childExercisesInfo = exports.getAllExercises(exercises[i].childExercises);
...
let _uploadContentImages = (exercise, iIndex, parentSequenceNum, jIndex) => {
    ...
    if (images != null) {
        for (let j = 0; j < images.length; j++) {
            ...
            let img = s3_1.parseAndUploadImage(images[j], sequence_num, exercise['path'], iIndex, jIndex);
            uploadPromises.push(img);
        }
    }
    return Promise.all(uploadPromises);
};
exports.uploadImagesAndUpdateContent = () => {
    let exPromises = [];
    let exChildPromises = [];
    for (var i = 0; i < globals.exercises.length; i++) {
        ...
        exPromises.push(_uploadContentImages(exercise, i).then((uploadedImages) => {
            if (uploadedImages.length) {
                ...
                globals.exercises[iIndex]['content'] = utils_2.updateContentWithImageLinks(uploadedImages, content);
            }
            ;
        }));
            ...
            for (var j = 0; j < childExercises.length; j++) {
                exChildPromises.push(_uploadContentImages(childExercises[j], i, exercise['sequence_num'], j).then((uploadedImages) => {
                    if (uploadedImages.length) {
                        ...
                        globals.exercises[iIndex]['childExercises'][jIndex]['content'] = utils_2.updateContentWithImageLinks(uploadedImages, content);
                    }
                    ;
                }));
            }
