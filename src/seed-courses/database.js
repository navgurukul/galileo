...
const _generateExerciseAddOrUpdateQuery = function (exerciseInfo) {
  const query = database('exercises')
    .where({ slug: exerciseInfo.slug })
    .then((rows) => {
      // a exercise with same slug exists
      if (rows.length > 0) {
        const dbReviewType = rows[0].reviewType;
        return database('exercises')
          .where({ id: rows[0].id })
          .update(exerciseInfo)
          .then(() => Promise.resolve(rows[0].id))
          .then((exerciseId) => {
            ...
            if (dbReviewType !== exerciseInfo.reviewType) {
              return database('submissions')
                ...
                .delete()
                .then(() => Promise.resolve(exerciseId));
            }
...
export const addOrUpdateCourse = function () {
  return database('courses')
    ...
    .then((row) => database('courses').select(database.raw('MAX(sequenceNum) as sequenceNum'))
      .then((rows) => {
        ...
        return database('courses')
          ...
          .update({
            ...
            sequenceNum: sequenceNum || newSequenceNum,
          })
          .then(() => Promise.resolve(courseId));
      }));
};
...
export const deleteExercises = function (courseId) {
  database('exercises')
    ...
    .then((dbSlugs) => {
      ...
      const slugDiff = dbSlugs.filter((x) => globals.allSlugs.indexOf(x) < 0);
          ...
          database('exercises')
            ...
            .then((rows) => {
              ...
              return database('submissions')
                ...
                .delete()
                .then(() => database('exercises')
                  .where({ slug: slugDiff[i] })
                  .delete());
            }),
        );
      }
      return Promise.all(deleteQueries).then(() => Promise.resolve(true));
    });
};
